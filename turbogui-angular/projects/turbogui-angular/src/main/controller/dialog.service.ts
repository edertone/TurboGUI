...
export class DialogService extends SingletoneStrictClass {
    ...
    addCloseApplicationWarning() {
        this._windowBeforeUnloadUnListen ??= this._renderer.listen(
            'window',
            'beforeunload',
     ...
     */
    addDialog(
        dialogComponentClass: Type<DialogBaseComponent>,
        properties: {
            id?: string;
            width?: string;
            maxWidth?: string;
        ...
        return new Promise(resolve => {
            ...
            if (className === '') {
                throw new Error(
                    `The static property DIALOG_CLASS_NAME is not defined or is empty for this dialog component (${dialogComponentClass})`
                );
            }
            ...
            dialogRef
                .beforeClosed()
                .pipe(take(1))
                .subscribe((selection: { index: number; value?: any }) => {
                    if (properties.scrollLock) {
     ...
     */
    private async _addFileBrowserDialogInternal(options: {
        multiple: boolean;
        accept: string;
        maxFileSize?: number;
        maxTotalSize?: number;
    ...
    private _disableUserInteraction() {
        ...
        this._documentMousedownUnlisten ??= this._renderer.listen('document', 'mousedown', event =>
            event.preventDefault()
        );

        this._documentPointerdownUnlisten ??= this._renderer.listen('document', 'pointerdown', event =>
            event.preventDefault()
        );
    }
